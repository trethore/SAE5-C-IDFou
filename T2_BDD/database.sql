-- ====================================================================================
-- FICHIER UNIQUE: SCHEMA (donné) + VUES + FONCTIONS/TRIGGERS
-- Basé sur le schema fourni par l'utilisateur. Ajouts minimes: table track_comment (pour les triggers de commentaires)
-- ====================================================================================
...
-- ------------------------------------------------
-- DROP EXISTANTS (séquence identique à ton dump initial)
-- ------------------------------------------------
...
-- ====================================================================================
-- TABLES (identiques au schéma fourni, inchangées)
-- ====================================================================================
...
DECLARE
    ...
    has_artist BOOLEAN;
BEGIN
    -- Récupérer l'artiste anonyme dans account ou le créer s'il n'existe pas
    ...
    IF anon_artist_id IS NULL THEN
        ...
        VALUES (uuid_generate_v4(), 'anonymous_artist', 'anon@example.com', 'Anonymous Artist', CURRENT_TIMESTAMP)
        RETURNING account_id INTO anon_artist_id;
    END IF;

    -- Vérifier si l'artiste anonyme existe dans la table artist, sinon créer
    IF NOT EXISTS (SELECT 1 FROM artist WHERE artist_id = anon_artist_id) THEN
        INSERT INTO artist(artist_id, artist_bio, artist_location, artist_latitude, artist_longitude,
                           artist_active_year_begin, artist_active_year_end, artist_favorites, artist_comments)
        VALUES (anon_artist_id, 'Anonymous Artist', NULL, NULL, NULL, NULL, NULL, 0, 0);
    END IF;

    -- Vérifier s'il existe déjà un main artist pour ce track
    SELECT EXISTS(
    ...
    IF NOT has_artist AND NOT EXISTS (
        ...
        -- Créer le lien avec l'artiste anonyme
...
-- Créer le trigger AFTER INSERT sur track
DROP TRIGGER IF EXISTS tr_track_assign_anonymous_if_no_main_artist_and_album_artists ON track;
