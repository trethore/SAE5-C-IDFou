        ...
        for column, rules in validation_rules.items():
            ...
            for rule in rules:
                if not _respect_rule(
                    ...
                    rule_failure_stats[rule_key] = (
                        rule_failure_stats.get(rule_key, 0) + 1
                    )
                    break
...
def clean_csv(
    ...
    *,
    ...
    standardisation_rules_config: dict[str, list[str]] = config.get(
        "standardisation_rules", {}
    )
    csv_name = csv_path.name

    header_rows = cast(
        Sequence[int] | int | Literal["infer"] | None, config.get("header_rows")
    )
    ...
    specific_validation_rules = {
        column: rules
        for column, rules in validation_rules_config.items()
        if column != "__all__"
    }
    ...
    specific_standardisers = {
        column: rules
        for column, rules in standardisation_rules_config.items()
        if column != "__all__"
    }
    ...
    if default_validation_rules:
        ...
        selected_columns = [
            col for col in dataframe.columns if col in specific_validation_rules
        ]
    ...
    for column, rules in effective_validation_rules.items():
        dataframe.loc[:, column] = dataframe[column].apply(
            lambda value: _convert_value(value, rules)
        )

    valid_indices, rule_failures = _validate_dataframe(
        dataframe, effective_validation_rules
    )
    ...
    retention_percentage = (
        (cleaned_row_count / filtered_row_count) * 100
