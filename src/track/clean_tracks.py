...
def clean_csv(csv_path: str, rules_by_csv: dict, show_stats: bool = False) -> None:
    ...
    print(f"After column filtering: {filtered_row_count}")
    ...
    for row_index, row in dataframe.iterrows():
        ...
        for column in dataframe.columns:
            if column in column_rules:
                for rule in column_rules[column]:
                    if not respect_rule(row[column], rule, row):
                        row_is_valid = False
    ...
    retention_percentage = (cleaned_row_count / filtered_row_count * 100) if filtered_row_count > 0 else 0
    
    OUTPUT_DIR.mkdir(exist_ok=True)
    output_path = OUTPUT_DIR / f"clean_{csv_filename}"
    cleaned_dataframe.to_csv(output_path, index=False)
    
    print(f"After validation rules: {cleaned_row_count} (removed {removed_row_count}, {retention_percentage:.2f}% retained)")
    print(f"Cleaned CSV saved to: {output_path}")
    
    if show_stats and rule_failure_stats:
        print("\n=== Rule Failure Statistics ===")
        sorted_failures = sorted(rule_failure_stats.items(), key=lambda x: x[1], reverse=True)
        for rule_key, failure_count in sorted_failures:
            print(f"{rule_key}: {failure_count} failures")
...
if __name__ == "__main__":
    ...
    clean_csv(CSV_PATH, RULES_BY_CSV, show_stats)
